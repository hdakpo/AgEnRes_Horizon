# ============================
# ESTIMATION FERT GC — ESSAIS DÉCROISSANTS (K = nb_cultures .. 3)
...
# ---- Paramètres généraux ----
eps <- 1e-8  # évite zéros stricts CROs alpha
...
# ---- Lecture des données ----
input_file <- file.path("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/input/Data_for_allocation",
                        paste0("prepared_list_FERT_GC_", CURRENT_COUNTRY_CODE, ".RDS"))
...
stopifnot(all(c("ID","YEAR","FERT_N_HA") %in% names(MYDATA_GC)))
MYDATA_GC <- MYDATA_GC %>%
  mutate(ID = as.character(ID), YEAR = as.integer(YEAR)) %>%
  filter(!is.na(ID), trimws(ID)!="", !is.na(YEAR))

# LOGNORMAL => garder FERT_N_HA > 0 & non-NA
if (anyNA(MYDATA_GC$FERT_N_HA)) {
  cat("⚠️ Retrait FERT_N_HA NA:", sum(is.na(MYDATA_GC$FERT_N_HA)),"\n")
  MYDATA_GC <- filter(MYDATA_GC, !is.na(FERT_N_HA))
}
if (any(MYDATA_GC$FERT_N_HA <= 0, na.rm=TRUE)) {
  cat("⚠️ Retrait FERT_N_HA <= 0:", sum(MYDATA_GC$FERT_N_HA <= 0, na.rm=TRUE), "\n")
  MYDATA_GC <- filter(MYDATA_GC, FERT_N_HA > 0)
}
...
try_fit_for_K <- function(K, base_df) {
  ...
  S <- pmax(S, 0) + eps
  S <- S / rowSums(S)
  if (any(!is.finite(S))) stop("NA/Inf CROs alpha (K=", K, ")")
  ...
  # Checks rapides
  used_cols <- unique(c("ID","YEAR","FERT_N_HA", cname_alloc_SH_red,
...
# À COLLER À LA PLACE DE TON BLOC PRÉDICTIONS ACTUEL
# ==========
...
pred_name_vec <- paste0(gc_vec, "_FERT_N")
# Ajustement de longueur si besoin
if (length(pred_name_vec) < pred_cols) {
  pred_name_vec <- c(pred_name_vec, paste0("extra", seq_len(pred_cols - length(pred_name_vec)), "_FERT_N"))
...
if (length(minor_crops_selected)) {
  ...
  if (OTHER_col %in% colnames(pred_df)) {
    ...
    if (length(minor_gc) == ncol(pred_minor)) {
      colnames(pred_minor) <- paste0(minor_gc, "_FERT_N")
    } else {
      warning("⚠️ pred_minor: ", ncol(pred_minor), " colonnes vs noms ", length(minor_gc), ". Noms génériques.")
      colnames(pred_minor) <- paste0("minor", seq_len(ncol(pred_minor)), "_FERT_N")
    }
    ...
    # Pas d'OTHER CROs le modèle (K == toutes les cultures)
    out_pred <- xit_pred
  }
} else {
...
outputdir <- "D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/output"

dir.create(outputdir, showWarnings = FALSE, recursive = TRUE)
save(list = c("MYDATA_GC_estimation","fit","MYDATA_GC","CURRENT_COUNTRY_CODE",
              "K_selected","top_crops_selected","minor_crops_selected"),
     file = file.path(outputdir, paste0("output_FERT_GC_Kdec_", CURRENT_COUNTRY_CODE, ".RData")))
