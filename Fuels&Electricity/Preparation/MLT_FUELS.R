...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_2017_2022/MLT.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FUELS_MLT.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CCEROTHALL",
                    "CPOTALL", 
                    "CVEGFLOW","CPERWOUTWINE", "CFODDOTHALL",
                    "CARAOTHALL")
...
CCEROTHALL_name <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "CRYE", "COAT", "CRICE","CCEROTHN")
CVEGFLOW_name <- "SE046"
CPERWOUTWINE_name <- "SE054"
CFODDOTHALL_name <- c("CGRSTMP", "CGRSXRG", "CFODRTBR", "CRG", "CFODDOTHN")
CARAOTHALL_name <- c("CSUGBT", "CRAPE", "CSNFL", "CSOYA", "SE050", "CARAOTHN")
...
MYDATA_GC <- MYDATA_GC %>%
  mutate(CPERWOUTWINE_A = rowSums(across(all_of(CPERWOUTWINE_name_A))),
...
share_path <- "D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data pour outliers treat/"
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_GC_MLT.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FUELS_MLT.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CCEROTHALL",
                    "CPOTALL", 
                    "CVEGFLOW","CPERWOUTWINE", "CWINE",
                    "CFODDOTHALL", "CARAOTHALL")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_LS_MLT.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FUELS_MLT.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CCEROTHALL",
                    "CPOTALL", 
                    "CVEGFLOW","CPERWOUTWINE", "CWINE",
                    "CFODDOTHALL", "CARAOTHALL")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_MX_MLT.RDS"))
