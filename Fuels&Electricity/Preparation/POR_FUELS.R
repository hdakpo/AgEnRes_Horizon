...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_2017_2022/POR.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FUELS_POR.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTALL", "CMZ", "COAT", "CRICE", "CCEROTHALL",
                    "CPOT", "CPOTOTH", 
                    "COILSEEDALL",
...
CCROPOBS_name <- c("CMZ", "COAT", "CRICE",
                   "CPOT", "CGRSTMP", "CGRSXRG", "CRG")

# Création des agrégées
CWHTALL_name <- c("CWHTC", "CWHTD")
CPOTOTH_name <- c("CPOTST", "CPOTOTH")
CCEROTHALL_name <- c("CBRL","CCEROTHN", "CRYE")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_GC_POR.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FUELS_POR.csv")
write.csv2(res, share_output_file)
...
CCROPOBS_name <- c("CMZ", "CRYE", "CGRSTMP", "CGRSXRG", "CRG")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_LS_POR.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FUELS_POR.csv")
write.csv2(res, share_output_file)
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_MX_POR.RDS"))
