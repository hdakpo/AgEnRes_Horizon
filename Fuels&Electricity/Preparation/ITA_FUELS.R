...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/FUels/input/Data_2017_2022/ITA.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FUELS_ITA.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "COAT", "CRICE", "CCEROTHALL",
                    "CPOT", "CPOTOTH",
                    "COILSEEDOTH", "CSOYA",
...
CCROPOBS_name <- c( "CWHTC", "CWHTD", "CMZ", "CBRL", "COAT", "CRICE","CPOT", "CSOYA", "CGRSTMP", "CGRSXRG")

# Création des agrégées
CPOTOTH_name <- c("CPOTST" , "CPOTOTH")
CCEROTHALL_name <- c("CRYE","CCEROTHN")
COILSEEDOTH_name <- c("CRAPE" , "CSNFL")
CVEGFLOW_name <- "SE046"
...
MYDATA_GC <- MYDATA_GC %>%
  mutate(CPOTOTH_A = rowSums(across(all_of(CPOTOTH_name_A))),
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_GC_ITA.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FUELS_ITA.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "COAT", "CCEROTHALL",
                    "CSOYA", 
                    "CPERWOUTWINE", 
                    "CGRSTMP", "CGRSXRG", "CRG", "CFODDOTHALL", 
                    "CARAOTHALL")

CCROPOBS_name <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "COAT", "CSOYA", "CGRSTMP", "CGRSXRG", "CRG")

# Création des agrégées
CCEROTHALL_name <- c("CRYE", "CRICE", "CCEROTHN")
CPERWOUTWINE_name <- "SE054"
CARAOTHALL_name <- c("CPOT", "CPOTST", "CPOTOTH", "CSUGBT", "CRAPE", "CSNFL", "SE046", "SE050", "CARAOTHN")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_LS_ITA.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FUELS_ITA.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "COAT", "CCEROTHALL",
                    "COILSEEDALL",
                    "CVEGFLOW", "CPERWOUTWINE", "CWINE", 
                    "CGRSTMP", "CGRSXRG", "CRG", "CFODDOTHALL", 
                    "CARAOTHALL")

CCROPOBS_name <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "COAT", "CGRSTMP", "CGRSXRG", "CRG")
...
CPERWOUTWINE_name <- "SE054"
CWINE_name <- "SE050"
CARAOTHALL_name <- c("CPOT", "CPOTST", "CPOTOTH", "CSUGBT", "CARAOTHN")
CFODDOTHALL_name <- c("CFODRTBR", "CFODDOTHN")
...
# Calculer les superficies des variables créées
MYDATA_MX <- MYDATA_MX %>%
  mutate(COILSEEDALL_A = rowSums(across(all_of(COILSEEDALL_name_A))),
         CCEROTHALL_A = rowSums(across(all_of(CCEROTHALL_name_A))),
         CVEGFLOW_A = rowSums(across(all_of(CVEGFLOW_name_A))),
         CPERWOUTWINE_A = rowSums(across(all_of(CPERWOUTWINE_name_A))),
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_MX_ITA.RDS"))
