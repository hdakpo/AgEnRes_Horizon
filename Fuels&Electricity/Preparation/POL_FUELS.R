...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_2017_2022/POL.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FUELS_POL.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTALL", "CMZ", "CBRL", "CRYE", "COAT", "CCEROTHALL",
                    "CPOTALL",
                    "CSUGBT", "COILSEEDALL",
                    "CVEGFLOW",
                    "CGRSXRG", "CFODDOTHALL",
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_GC_POL.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FUELS_POL.csv")
write.csv2(res, share_output_file)
...
CCROPOBS_name <- c("CMZ", "CBRL", "CRYE", "COAT", "CGRSTMP", "CGRSXRG")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_LS_POL.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FUELS_POL.csv")
write.csv2(res, share_output_file)
...
CPOTALL_name <- c("CPOT","CPOTST", "CPOTOTH")
COILSEEDALL_name <- c("CRAPE","CSNFL", "CSOYA")
CARAOTHALL_name <- c("SE046", "SE050", "SE054", "CARAOTHN")
CFODDOTHALL_name <- c("CFODRTBR", "CRG", "CFODDOTHN")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_MX_POL.RDS"))
