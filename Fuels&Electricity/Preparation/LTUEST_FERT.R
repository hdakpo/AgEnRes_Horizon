...
# Preparation share  -------------------------------------------
share_path <- "D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/SHARE/"
output_path <- "D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data pour outliers treat/"
# Chargement des données principales
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_2017_2022/LTUEST.RDS")
...
share_output_file <- paste0(share_path, "share_GC_FUELS_LTUEST.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTC" , "CWHTD","CMZ", "CBRL", "CRYE", "COAT", "CRICE", "CCEROTHALL",
                    "CPOTALL", 
...
CCROPOBS_name <- c("CWHTC" , "CWHTD","CMZ", "CBRL",  "CRYE", "COAT", "CRICE",
                   "CSUGBT", "CRAPE")

# Création des agrégées
CCEROTHALL_name <-  "CCEROTHN"
CPOTALL_name <- c("CPOT", "CPOTST", "CPOTOTH")
COILSEEDOTH_name <- c("CSNFL","CSOYA")
CVEGFLOW_name <- "SE046"
CPERWOUTWINE_name <- "SE054"
CARAOTHALL_name <- c( "SE050", "CARAOTHN")
CFODDOTHALL_name <- c("CFODRTBR", "CGRSTMP", "CGRSXRG", "CRG", "CFODDOTHN")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FUELS_GC_LTUEST.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(share_path, "share_LS_FUELS_LTUEST.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CWHTALL", "CMZ", "CBRL", "CCEROTHALL",
                    "CPERWOUTWINE",
                    "CGRSTMP", "CGRSXRG", "CRG", "CFODDOTHALL",
                    "CARAOTHALL")

CCROPOBS_name <- c("CMZ", "CBRL", "CGRSTMP", "CGRSXRG", "CRG")

# Création des agrégées
CWHTALL_name <- c("CWHTC" , "CWHTD")
CCEROTHALL_name <- c( "COAT","CRYE" , "CRICE", "CCEROTHN")
CARAOTHALL_name <- c("CPOT", "CPOTST", "CPOTOTH", "CSUGBT", "CRAPE","CRYE", "CSNFL", "SE046", "SE050",  "CARAOTHN")
CPERWOUTWINE_name <- "SE054"
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FUELS_LS_LTUEST.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(share_path, "share_MX_FUELS_LTUEST.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CWHTD", "CWHTC", "CMZ", "CBRL", "COAT", "CCEROTHALL",
                    "CPERWOUTWINE", 
                    "CRAPE", "COILSEDOTH",
                    "CGRSTMP", "CGRSXRG", "CRG","CFODDOTHALL",
                    "CARAOTHALL")

CCROPOBS_name <- c( "CWHTD", "CWHTC", "CMZ", "CBRL","COAT", "CRAPE", 
                    "CGRSTMP", "CGRSXRG", "CRG")

# Création des agrégées
CCEROTHALL_name <- c( "CRYE",  "CRICE", "CCEROTHN")
CPERWOUTWINE_name <- "SE054"
COILSEDOTH_name <- c("CSNFL", "CSOYA")
CFODDOTHALL_name <- c("CFODRTBR", "CFODDOTHN")
CARAOTHALL_name <- c("CPOT", "CPOTST", "CPOTOTH", "CSUGBT", "SE046",  "SE050", "CARAOTHN")

cname_alloc_A <- paste0(cname_alloc_mx, sep="_", "A")
CCEROTHALL_name_A <- paste0(CCEROTHALL_name, sep="_", "A")
COILSEDOTH_name_A <- paste0(COILSEDOTH_name, sep="_", "A")
CPERWOUTWINE_name_A <- paste0(CPERWOUTWINE_name, sep="_", "A")
CARAOTHALL_name_A <- paste0(CARAOTHALL_name, sep="_", "A")
CFODDOTHALL_name_A <- paste0(CFODDOTHALL_name, sep="_", "A")
...
MYDATA_MX <- MYDATA_MX %>%
  mutate(CCEROTHALL_A = rowSums(across(all_of(CCEROTHALL_name_A))),
         COILSEDOTH_A = rowSums(across(all_of(COILSEDOTH_name_A))),
         CPERWOUTWINE_A = rowSums(across(all_of(CPERWOUTWINE_name_A))),
         CFODDOTHALL_A = rowSums(across(all_of(CFODDOTHALL_name_A))),
         CARAOTHALL_A = rowSums(across(all_of(CARAOTHALL_name_A))))
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FUELS_MX_LTUEST.RDS"))
