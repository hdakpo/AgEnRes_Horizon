setwd("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels")
output_path <- "D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/SHARE/"

# Preparation share pour FRANCE -------------------------------------------
eu_countries <- c(
  "BEL", "BGR", "CYP", "CZE", "DAN", "DEU", "ELL", "ESP", "EST", 
...
if(.Platform$GUI == "RStudio"){
  # Configuration pour test local dans RStudio
  CURRENT_COUNTRY_CODE <- "FRA"  # <-- MODIFIER ICI POUR CHANGER DE PAYS EN LOCAL
  JOBS <- 1
} else {
  # Configuration pour cluster
  JOBS <- as.integer(args[[1]])
  CURRENT_COUNTRY_CODE <- eu_countries[JOBS]
}
# Chargement des données principales
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_2017_2022/FRA.RDS")
...
output_path <- "input/SHARE/"
share_output_file <- paste0(output_path, "share_GC_FUELS_", CURRENT_COUNTRY_CODE, ".csv")
if (!dir.exists(output_path)) {
  dir.create(output_path, recursive = TRUE)
  cat("✅ Dossier créé :", normalizePath(output_path), "\n")
}
...
share_path <- "input/Data pour outliers treat/"
saveRDS(mylist_for_model, file =  paste0(share_path, "prepared_objects_FUELS_GC_", CURRENT_COUNTRY_CODE, ".RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FUELS_", CURRENT_COUNTRY_CODE, ".csv")
write.csv2(res, share_output_file)
...
share_path <- "input/Data pour outliers treat/"
saveRDS(mylist_for_model, file =  paste0(share_path, "prepared_objects_FUELS_LS_", CURRENT_COUNTRY_CODE, ".RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FUELS_", CURRENT_COUNTRY_CODE, ".csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CWHTALL", "CMZ", "CBRL", "CCEROTHALL",
                    "CPOTALL", "CSUGBT",
                    "CRAPE", "COILSEEDOTH",
                    "CVEGFLOW", 
                    "CGRSTMP", "CGRSXRG", "CFODDOTHALL",
...
share_path <- "input/Data pour outliers treat/"
saveRDS(mylist_for_model, file =  paste0(share_path, "prepared_objects_FUELS_MX_", CURRENT_COUNTRY_CODE, ".RDS"))
