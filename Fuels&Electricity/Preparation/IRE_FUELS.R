...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_2017_2022/IRE.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FUELS_IRE.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTALL", "CBRL", "COAT", "CCEROTHALL",
                    "COILSEEDALL",
                    "CGRSXRG", "CFODDOTHALL", 
                    "CARAOTHALL")

CCROPOBS_name <- c( "CBRL","COAT", "CGRSXRG")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_GC_IRE.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FUELS_IRE.csv")
write.csv2(res, share_output_file)
...
CCROPOBS_name <- c("CGRSXRG", "CRG")

# Création des agrégées
CCEROTHALL_name <- c("CWHTC" , "CWHTD", "CMZ", "CBRL", "CRYE", "COAT", "CRICE", "CCEROTHN")
CARAOTHALL_name <- c("CRAPE", "CSNFL", "CSOYA", "CPOT", "CPOTST", "CPOTOTH", "CSUGBT", "SE046", "SE054" , "SE050", "CARAOTHN")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_LS_IRE.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FUELS_IRE.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CWHTALL", "COAT", "CBRL", "CCEROTHALL",
                    "CFODRTBR", "CGRSXRG", "CFODDOTHALL", 
                    "CARAOTHALL")

CCROPOBS_name <- c("COAT", "CBRL", "CFODRTBR", "CGRSXRG")

# Création des agrégées
CWHTALL_name <- c("CWHTC" , "CWHTD")
CCEROTHALL_name <- c("CMZ", "CRYE", "CRICE", "CCEROTHN")
CARAOTHALL_name <- c("CPOT", "CPOTST", "CPOTOTH", "CSUGBT", "CRAPE", "CSNFL", "CSOYA", "SE046", "SE054", "SE050", "CARAOTHN")
CFODDOTHALL_name <- c("CGRSTMP", "CRG", "CFODDOTHN")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_MX_IRE.RDS"))
