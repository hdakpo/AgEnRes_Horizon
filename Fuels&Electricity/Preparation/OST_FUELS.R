...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_2017_2022/OST.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FUELS_OST.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "CRYE", "CCEROTHALL",
                    "CPOTALL",
                    "CSUGBT", "CRAPE", "CSNFL", "CSOYA",
                    "CVEGFLOW",
                    "CGRSXRG", "CFODDOTHALL", 
                    "CARAOTHALL")

CCROPOBS_name <- c("CWHTC", "CWHTD", "CMZ", "CBRL", "CRYE",
                   "CSUGBT", "CRAPE", "CSNFL", "CSOYA", "CGRSXRG")

# Création des agrégées
CPOTALL_name <- c("CPOT", "CPOTST", "CPOTOTH")
CCEROTHALL_name <- c("CRICE","COAT", "CCEROTHN")
CVEGFLOW_name <- "SE046"
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_GC_OST.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FUELS_OST.csv")
write.csv2(res, share_output_file)
...
CARAOTHALL_name <- c("CPOT", "CPOTST" , "CPOTOTH", "CSUGBT", "SE050", "SE054", "SE046","CARAOTHN")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_LS_OST.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FUELS_OST.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CWHTALL", "CMZ", "CBRL", "CRYE", "COAT", "CCEROTHALL",
                    "CPOTALL",
                    "CSUGBT", "CRAPE", "CSOYA", 
                    "CPERWOUTWINE", 
...
CCROPOBS_name <- c("CMZ","CBRL", "CRYE", "COAT",
                   "CSUGBT", "CRAPE", "CSOYA", 
                   "CGRSTMP", "CGRSXRG")

# Création des agrégées
CWHTALL_name <- c("CWHTC", "CWHTD")
CCEROTHALL_name <- c("CRICE","CCEROTHN")
CPOTALL_name <- c("CPOT","CPOTST", "CPOTOTH")
CPERWOUTWINE_name <- "SE054"
CARAOTHALL_name <- c("CSNFL", "SE046", "SE050", "CARAOTHN")
...
saveRDS(mylist_for_model, file = paste0(share_path, "prepared_objects_FUELS_MX_OST.RDS"))
