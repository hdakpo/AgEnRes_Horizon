# ============================
# 🚀 TRAITEMENT DES OUTLIERS – LTUEST SEUL
# ============================
...
# ----------------------------
# Paramètres spécifiques LTUEST
# ----------------------------
code <- "LTUEST"

input_file  <- "D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data pour outliers treat/prepared_objects_FUELS_GC_LTUEST.RDS"
out_dir <- "D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Fuels/input/Data_for_allocation"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

output_file <- file.path(out_dir, "prepared_list_FUELS_GC_LTUEST.RDS")
...
if (!"COUNTRY" %in% names(MYDATA_GC)) {
  stop("La colonne COUNTRY est absente de MYDATA_GC (LTUEST regroupe probablement ELL et CYP).")
}
MYDATA_GC <- MYDATA_GC %>%
  mutate(
    COUNTRY = as.character(COUNTRY),
