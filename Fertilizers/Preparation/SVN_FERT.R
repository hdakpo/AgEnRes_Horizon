...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/input/Data_2017_2022/SVN.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FERT_SVN.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTALL", "CBRL", "CMZ", "COAT", "CCEROTHALL",
                    "CPOTALL", 
                    "CRAPE", "COILSEEDOTH",
                    "CVEGFLOW", "CPERWOUTWINE", "CWINE",
                    "CARAOTHALL")

CCROPOBS_name <- c("CBRL", "CMZ", "COAT",  
                  "CRAPE")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_GC_SVN.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FERT_SVN.csv")
write.csv2(res, share_output_file)
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_LS_SVN.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FERT_SVN.csv")
write.csv2(res, share_output_file)
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_MX_SVN.RDS"))
