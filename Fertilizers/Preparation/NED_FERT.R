...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/input/Data_2017_2022/NED.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FERT_NED.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTALL", "CBRL",  "CCEROTHALL",
                    "CPOT", "CPOTST", "CPOTOTH",
                    "CSUGBT", 
                    "CVEGFLOW",
                    "CARAOTHALL")

CCROPOBS_name <- c("CBRL", "CPOT", "CPOTST", "CPOTOTH",
                   "CSUGBT")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_GC_NED.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FERT_NED.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CWHTALL", "CCEROTHALL",
                    "CPOTALL", "CSUGBT", 
                    "CFODDER",
                    "CARAOTHALL")

CCROPOBS_name <- c("CSUGBT")

# Création des agrégées
CWHTALL_name <- c("CWHTC", "CWHTD")
CPOTALL_name <- c("CPOT", "CPOTST" , "CPOTOTH")
CCEROTHALL_name <- c( "CMZ", "CBRL", "CRYE", "COAT", "CRICE","CCEROTHN")
CARAOTHALL_name <- c("CRAPE", "CSNFL", "CSOYA", "SE050", "SE054", "SE046","CARAOTHN")
CFODDER_name <- "SE071"
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_LS_NED.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FERT_NED.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CWHTALL", "CBRL", "CCEROTHALL",
                    "CPOT", "CPOTST", "CPOTOTH",
                    "CSUGBT",
                    "CVEGFLOW", "CPERWOUTWINE", "CFODDER",
                    "CARAOTHALL")
...
CARAOTHALL_name <- c("CRAPE", "CSNFL", "CSOYA", "SE050", "CARAOTHN")
CFODDER_name <- "SE071"
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_MX_NED.RDS"))
