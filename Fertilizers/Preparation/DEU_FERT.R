...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/input/Data_2017_2022/DEU.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FERT_DEU.csv")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_GC_DEU.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FERT_DEU.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CWHTALL", "CMZ", "CBRL", "CRYE", "CCEROTHALL",
                    "COILSEEDALL",
                    "CFODDER",
                    "CARAOTHALL")

CCROPOBS_name <- c( "CMZ", "CBRL", "CRYE")

# Création des agrégées
CWHTALL_name <- c("CWHTC" , "CWHTD")
CCEROTHALL_name <- c("COAT" , "CRICE", "CCEROTHN")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_LS_DEU.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FERT_DEU.csv")
write.csv2(res, share_output_file)
...
CCROPOBS_name <- c("CMZ", "CBRL", "CRYE", "COAT", "CSUGBT")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_MX_DEU.RDS"))
