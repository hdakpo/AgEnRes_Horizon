...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/input/Data_2017_2022/CZE.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FERT_CZE.csv")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_GC_CZE.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FERT_CZE.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CWHTC", "CBRL", "COAT", "CCEROTHALL",
                    "COILSEEDALL",
                    "CFODDER",
                    "CARAOTHALL")

CCROPOBS_name <- c("CWHTC", "CBRL", "COAT")

# Création des agrégées
CCEROTHALL_name <- c("CWHTD", "CMZ" , "CRYE", "CRICE", "CCEROTHN")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_LS_CZE.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FERT_CZE.csv")
write.csv2(res, share_output_file)
...
CCROPOBS_name <- c("CMZ", "CBRL", "COAT", "CSUGBT", "CRAPE")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_MX_CZE.RDS"))
