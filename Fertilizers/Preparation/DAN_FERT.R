...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/input/Data_2017_2022/DAN.RDS")
...
share_output_file <- paste0(output_path, "share_GC_FERT_DAN.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_gc <- c("CWHTALL", "CBRL", "CRYE", "COAT",
                    "CPOT", "CPOTOTH", 
                    "CSUGBT", "COILSEEDALL",  
                     "CARAOTHALL")
...
CARAOTHALL_name <- c("CMZ", "CRICE", "CCEROTHN","SE046", "SE054", "SE050", "CARAOTHN")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_GC_DAN.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FERT_DAN.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CWHTALL", "CBRL", "CRYE", "COAT", "CCEROTHALL",
                    "CPOTALL", 
                    "COILSEEDALL",
                    "CFODDER",
                    "CARAOTHALL")
...
CARAOTHALL_name <- c("CSUGBT", "SE046", "SE050" , "SE054", "CARAOTHN")
CFODDER_name <- "SE071"
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_LS_DAN.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FERT_DAN.csv")
write.csv2(res, share_output_file)
...
CCROPOBS_name <- c("CBRL", "CRYE", "COAT", "CPOT", "CPOTST", "CSUGBT")

# Création des agrégées
CWHTALL_name <- c("CWHTC" , "CWHTD")
COILSEEDALL_name <- c("CRAPE" ,  "CSNFL", "CSOYA")
CCEROTHALL_name <- c( "CMZ", "CRICE", "CCEROTHN")
CARAOTHALL_name <- c("CPOTOTH", "SE046","SE054", "SE050", "CARAOTHN")
CFODDER_name <- "SE071"
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_MX_DAN.RDS"))
