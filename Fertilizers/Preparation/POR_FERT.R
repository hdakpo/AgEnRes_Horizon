...
MYDATA <- readRDS("D:/Partage_Utilisateurs/RICA/TEST CLUSTER/Test cluster/input/Data_2017_2022/POR.RDS")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_GC_POR.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_LS_FERT_POR.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_ls <- c("CMZ", "CRYE", "CCEROTHALL",
                    "CPOTALL", 
                    "CPERWOUTWINE", "CWINE", 
                    "CFODDER",
                    "CARAOTHALL")

CCROPOBS_name <- c("CMZ", "CRYE")

# Création des agrégées
CPOTALL_name <- c("CPOT", "CPOTST" , "CPOTOTH")
CCEROTHALL_name <- c("CWHTC", "CWHTD", "CBRL", "COAT", "CRICE","CCEROTHN")
CARAOTHALL_name <- c( "CSUGBT", "CRAPE", "CSNFL", "CSOYA", "SE046","CARAOTHN")
CPERWOUTWINE_name <- "SE054"
CWINE_name <- "SE050"
CFODDER_name <- "SE071"
...
CPERWOUTWINE_name_A <- paste0(CPERWOUTWINE_name, sep="_", "A")
CWINE_name_A <- paste0(CWINE_name, sep="_", "A")
CFODDER_name_A <- paste0(CARAOTHALL_name, sep="_", "A")
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_LS_POR.RDS"))
...
# Sauvegarde des parts GC par pays
share_output_file <- paste0(output_path, "share_MX_FERT_POR.csv")
write.csv2(res, share_output_file)

# Regroupement pour allocation
cname_alloc_mx <- c("CMZ", "CRYE", "COAT", "CCEROTHALL",
                    "CPOT", "CPOTOTH",
                    "CVEGFLOW", "CPERWOUTWINE", "CWINE",
                    "CFODDER",
                    "CARAOTHALL")

CCROPOBS_name <- c("CMZ","CRYE", "COAT",
                   "CPOT")

# Création des agrégées
CCEROTHALL_name <- c("CWHTC", "CWHTD", "CBRL", "CRICE","CCEROTHN")
CPOTOTH_name <- c("CPOTST", "CPOTOTH")
CVEGFLOW_name <- "SE046"
CPERWOUTWINE_name <- "SE054"
CWINE_name <- "SE050"
CARAOTHALL_name <- c("CSUGBT", "CRAPE","CSNFL", "CSOYA", "CARAOTHN")
CFODDER_name <- "SE071"
...
# Calculer les superficies des variables créées
MYDATA_MX <- MYDATA_MX %>%
  mutate(CVEGFLOW_A = rowSums(across(all_of(CVEGFLOW_name_A))),
         CCEROTHALL_A = rowSums(across(all_of(CCEROTHALL_name_A))),
         CPOTOTH_A = rowSums(across(all_of(CPOTOTH_name_A))),
         CPERWOUTWINE_A = rowSums(across(all_of(CPERWOUTWINE_name_A))),
         CWINE_A = rowSums(across(all_of(CWINE_name_A))),
...
saveRDS(mylist_for_model, file = paste0(output_path, "prepared_objects_FERT_MX_POR.RDS"))
